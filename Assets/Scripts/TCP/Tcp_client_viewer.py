...
# -----------------------------
SERVER_IP = "192.168.1.136" # Change this to your server's IP address.
        ...
        while not stop_event.is_set():
            ...
            if sock is None:
                try:
                    ...
                    stop_event.wait(max(0.0, 0.5 - (time.time() - t_iter_start)))
                    continue
            ...
            stop_event.wait(max(0.0, send_period - (time.time() - t_iter_start)))
    finally:
        if sock is not None:
            try:
                sock.close()
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                try:
                    ...
                    pass
                sock = None
                stop_event.wait(max(0.0, 0.2 - (time.time() - t_iter_start)))
                continue

            if data == b"NO_FRAME":
                with state_lock:
                    state["status_text"] = "No frame"
                stop_event.wait(max(0.0, loop_sleep - (time.time() - t_iter_start)))
                continue
