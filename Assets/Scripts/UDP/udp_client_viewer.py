...
def consume_datagram(data: bytes, frames: dict, now: float):
    ...
    if info["chunk_count"] != chunk_count:
        # Inconsistent metadata for same frame id -> drop that frame.
        del frames[frame_id]
        return None
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                # Keep loop alive; transient network errors should not crash the UI thread.
                pass
